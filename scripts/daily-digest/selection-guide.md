...
- metrics はソースごとに単位が違う（YouTube は再生数、はてなはブックマーク数、Bluesky はいいね数、pixiv はランキングの順位と評価数、Steam は同時接続数か割引率）。ソースをまたいで数字の大小で比べない。
- metrics.sharers があれば、その動画を X で共有したアカウントの数（重複なし）。再生数が少なくても sharers が多ければ界隈で話題になっている（X を使っている間だけ）。
...
- 風景写真には、花・植物・動物・空など自然を写した写真も入れてよい。人物が主役の写真（ポートレート・コスプレ）や、まとめ投稿（多数の写真の寄せ集め）は外す。
