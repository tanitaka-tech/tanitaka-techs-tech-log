...
`pnpm digest render` はプレビュー用の記事を書き出し、候補をすべて載せます。`pnpm dev` で開くと、項目ごとに「✅ 採用 / ⛔ 不採用」のトグルが出ます。切り替えると `selection.json` に保存されます（`scripts/daily-digest/dev-review.mjs` が開発サーバーにだけ API を足します）。`publish` は採用した項目だけで公開用の記事に書き直してからコミットします。
