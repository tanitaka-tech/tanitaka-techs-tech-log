...
X の読み取りは従量課金なので、読んだ件数のわりに候補が残らないジャンルは検索を見直します。`review` の最後に、ジャンルごとの歩留まり（X 読み取り → 候補 → ルール適用後 → 掲載）が出ます。

- **人気の投稿を拾う**（`sampleRetweets: true`）: X の検索はいいね数順に並べられないので、キーワードに当たる投稿が多いジャンル（ai・illust・landscape）は、リツイートを新しい順に読み、よくリツイートされている投稿を拾う。リツイート元は同じ応答に含まれるので追加の読み取りは要らない（リツイート元も読み取りとして数える）
