...
`pnpm digest render` はプレビュー用の記事を書き出し、候補をすべて載せます。`pnpm dev` で開くと、項目ごとに「✅ 採用 / ⛔ 不採用」のトグルが出ます。切り替えると `selection.json` に保存されます（`scripts/daily-digest/dev-review.mjs` が開発サーバーにだけ API を足します）。目次のドラッグか「↑ 前へ / ↓ 後ろへ」で項目を並べ替えることもでき、公開する記事もその順になります。`publish` は採用した項目だけで公開用の記事に書き直してからコミットします。
...
風景写真・ツールの使い方・表現の解説・やる気が出るは、代わりの取得元が見つからないので休止しています（Pinterest はログインなしでは検索できず、公式 API の検索も提携企業向けに限られていた）。
